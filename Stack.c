// stack.c ... simple Stack of LocationIDs
...
void  pushOnto(Stack,LocationID);
LocationID popFrom(Stack);
...
void pushOnto(Stack s, LocationID src)
{
...
LocationID popFrom(Stack s)
{
	assert (s != NULL && s->top != NULL);
    LocationID src = s->top->val;
	Link old = s->top;
	s->top = old->next;
	free(old);
	return str;
